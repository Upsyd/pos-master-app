...
public class PosOrder extends OModel {
  ...
  //  OColumn account_move = new OColumn("Journal Entry",posOrder.class, OColumn.RelationType.ManyToOne);
...
//    OColumn invoice_id = new OColumn("Invoice",Invoice.class, OColumn.RelationType.ManyToOne);
   // OColumn lines = new OColumn("Order Lines", PosOrderLine.class, OColumn.RelationType.OneToMany);
//    OColumn location_id = new OColumn("Location",Location.class, OColumn.RelationType.ManyToOne);
    OColumn name = new OColumn("Order Ref", OVarchar.class);
    OColumn nb_print = new OColumn("Number of Prints", OInteger.class);
    OColumn note = new OColumn("Internal Notes", OVarchar.class);
    OColumn partner_id = new OColumn("Customer", ResPartner.class, OColumn.RelationType.ManyToOne);
  //  OColumn picking_id = new OColumn("Picking",posOrder.class, OColumn.RelationType.ManyToOne);
//    OColumn picking_type_id = new OColumn("Picking Type", posOrder.class, OColumn.RelationType.ManyToOne);
    OColumn pos_refrence = new OColumn("Receipt Ref", OVarchar.class);
 //   OColumn priselist_id = new OColumn("Pricelist",posOrder.class, OColumn.RelationType.ManyToOne);
   // OColumn sale_journal = new OColumn("Sale Journal", posOrder.class, OColumn.RelationType.ManyToOne);
    OColumn sequence_no = new OColumn("Sequence Number", OInteger.class);
   // OColumn session_id = new OColumn("Session", posOrder.class, OColumn.RelationType.ManyToOne);
    OColumn state= new OColumn("Status", OSelection.class);
   // OColumn statement_ids = new OColumn("Payments", posOrder.class, OColumn.RelationType.OneToMany);
    OColumn user_id = new OColumn("Salesman",ResPartner.class, OColumn.RelationType.ManyToOne);
